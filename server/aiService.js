...
async function answerQuery(userPrompt) {
    ...
    if (promptLower.includes('why was this block selected') || promptLower.includes('why b-102') || promptLower.includes('why was the block selected')) {
        if (plan && plan.block_id && plan.scheduled_tasks && plan.scheduled_tasks.length > 0) {
...
3. **Task Compatibility:** It allows concurrent track access for multiple engineering departments under a single possession.`;
            ...
            answerText = `No recommended block has been selected yet because there are currently no eligible maintenance requests or available block windows in the system.`;
        }
...
*(Note: Recommendation derived deterministically from live train headways and block capacity. The authorized human planner decides the final block assignment.)*`;
        ...
        const unscheduled = plan ? (plan.unscheduled_tasks || []) : [];
        if (unscheduled.length > 0) {
            ...
            answerText = plan && plan.corridor 
                ? `All eligible pending requests for ${plan.corridor} were accommodated in the block window.`
                : `No unscheduled requests found.`;
        }

    } else {
        if (plan && plan.plan_id && plan.scheduled_tasks && plan.scheduled_tasks.length > 0) {
            answerText = `Raksha AI Decision Support Summary:
• **Target Corridor:** ${plan.corridor}
...
• **System Role:** Prototype decision-support system requiring authorized human railway planner approval before publication.`;
            ...
            answerText = `Raksha AI Decision Support Summary:
• **System Status:** No active block plan recommendation currently generated.
• **Database State:** ${requests.length} maintenance requests, ${db.getAllWindows().length} block windows available.
• **Guidance:** Create maintenance work orders and configure block windows to initiate automated planning.`;
        }
    }
